...
/**
 * Helper to extract string value from Firestore raw field object.
 * Firestore raw format: { stringValue: "some text" } or { nullValue: null }
 */
...
functions.cloudEvent('statusChangeNotification', async (event) => {
  try {
    ...
    if (!data || !data.value || !data.oldValue) {
      console.log('Invalid event data: missing value or oldValue.');
      return;
    }
    ...
    // or sometimes full path: "projects/PROJECT_ID/databases/(default)/documents/jobs/JOB_ID"
    ...
    const message = {
      ...
      data: {
        jobId: jobId,
        url: '/' // Consistent with existing notification, though click handler logic is skipped for now
      },
      tokens: uniqueTokens
    };
    ...
    // We intentionally do not rethrow the error to prevent infinite retries if the logic itself is buggy.
    // However, for transient network issues, rethrowing might be desired.
    // Given this is a notification system, silent failure after logging is often safer than retry loops.
  }
});
